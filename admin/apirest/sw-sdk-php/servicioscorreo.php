...
$cte = $_POST['cliente'];
  ...
  $Query = "SELECT * FROM servicios WHERE idcliente='{$cte}' order by folio desc limit 1;";
  $consultad = $db->prepare($Query);
  $consultad->execute();
  while($filau = $consultad->fetch(PDO::FETCH_OBJ)){
    $res[] = $filau;
  }
  $servicio= (array)$res[0];
  ...
  $producto=$producto1['nombre'];
		  ...
		  $enviado = sendMail($nombre,$correo,$importe,$fecha, $tarjeta,$litros,$producto,$NomEstacion);
			if($enviado){
				echo "Enviado";
			}else{
        echo "0";
...
function sendMail($nombre,$correo,$importe,$fecha, $tarjeta,$litros,$producto,$NomEstacion){
    try{
                      ...
                      <tr>
                        <td>
                          <div class="text" style="text-align: center;">
                            <h2 style="color: #000;">Notificaci&oacute;n de Servicio</h2>
                            <p><b>Le informamos que se ha realizado un servicio con las siguientes caracter&iacute;sticas.</b></p>
                            <p><b>Fecha: '.formatDate($fecha).'</b></p>
                            <p><b>Estaci&oacute;n: '.$NomEstacion.'</b></p>
                            <p><b>Producto: '.$producto.'</b></p>
                            <p><b>Litros: '.$litros.'</b></p>
                            <p><b>Importe: $'.money_format('%i', $importe).'</b></p>
                            <p><b>Tarjeta: ************'.$tarjeta.'</b></p>
